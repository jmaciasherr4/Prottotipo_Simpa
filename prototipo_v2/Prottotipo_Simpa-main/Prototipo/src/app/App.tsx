...
// ── Types ─────────────────────────────────────────────────────────────────────
type Screen = "login" | "dashboard" | "analysis" | "labor" | "lot" | "gps" | "reports" | "alerts" | "personal" | "gestion";
...
const LABOR_TYPES = ["Chapia", "Corona", "Polinización", "Control químico de malezas", "Eliminación de flores andróginas", "Sanidad vegetal", "Abastecimiento"];
...
const NAV_ITEMS: { id: Screen; label: string; icon: React.ElementType; roles?: Role[] }[] = [
  ...
  { id: "gestion", label: "Gestión agrícola", icon: TreePine },
];
    ...
    <div className="min-h-screen bg-background flex">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="flex-1 overflow-y-auto">
          ...
          {screen === "gestion" && <ManagementScreen role={role} />}
          {screen === "personal" && (
            role === "Administrador"
...
// ── GESTIÓN AGRÍCOLA / REQUISITOS MUST ───────────────────────────────────────
function ManagementScreen({ role }: { role: Role }) {
  const [tab, setTab] = useState("variedades");
  const [varieties, setVarieties] = useState(() => JSON.parse(localStorage.getItem("simpa_variedades_v1") || "[{\"id\":1,\"nombre\":\"Guineensis\",\"umbralMadurez\":75,\"umbralPolinizacion\":80},{\"id\":2,\"nombre\":\"Híbrido OxG\",\"umbralMadurez\":78,\"umbralPolinizacion\":82} ]"));
  const [plans, setPlans] = useState(() => JSON.parse(localStorage.getItem("simpa_planes_v1") || "[]"));
  const [tariffs, setTariffs] = useState(() => JSON.parse(localStorage.getItem("simpa_tarifas_v1") || "[{\"id\":1,\"labor\":\"Chapia\",\"unidad\":\"ha\",\"tarifa\":45},{\"id\":2,\"labor\":\"Polinización\",\"unidad\":\"ha\",\"tarifa\":38},{\"id\":3,\"labor\":\"Corona\",\"unidad\":\"ha\",\"tarifa\":30}]"));
  const [advances, setAdvances] = useState(() => JSON.parse(localStorage.getItem("simpa_avances_v1") || "[]"));
  const [approved, setApproved] = useState(false);
  const [toast, setToast] = useState("");
  const [production, setProduction] = useState(49);
  const [greenFruit, setGreenFruit] = useState(14);
  const [maturity, setMaturity] = useState(82);

  useEffect(() => { localStorage.setItem("simpa_variedades_v1", JSON.stringify(varieties)); }, [varieties]);
  useEffect(() => { localStorage.setItem("simpa_planes_v1", JSON.stringify(plans)); }, [plans]);
  useEffect(() => { localStorage.setItem("simpa_tarifas_v1", JSON.stringify(tariffs)); }, [tariffs]);
  useEffect(() => { localStorage.setItem("simpa_avances_v1", JSON.stringify(advances)); }, [advances]);

  const notify = (m: string) => { setToast(m); setTimeout(() => setToast(""), 2500); };
  const tabs = [
    ["variedades", "RF-10 · Variedades y umbrales"],
    ["nutricion", "RF-08 · Diagnóstico nutricional"],
    ["produccion", "RF-18 · Estimación de producción"],
    ["madurez", "RF-21/RF-22 · Madurez y fruta verde"],
    ["planificacion", "RF-26 · Planificación semanal"],
    ["avance", "RF-35 · Avance delegado"],
    ["tarifas", "RF-36 · Tarifas por labor"],
    ["remuneracion", "RF-37 · Remuneración semanal"],
    ["derechos", "RF-40/41/42 · Derechos LOPDP"],
  ];
  const card = "bg-card border border-border rounded-2xl p-4";
  const btn = "rounded-xl px-3 py-2 text-sm font-bold transition-colors";

  const addPlan = () => {
    const newPlan = { id: Date.now(), semana: `Semana ${plans.length + 1}`, lot: "Lote 1", labor: "Polinización", meta: 120, presupuesto: 456, estado: "Planificada" };
    setPlans([newPlan, ...plans]); notify("Plan semanal creado");
  };
  const addAdvance = () => {
    const item = { id: Date.now(), lot: "Lote 1", worker: "Trabajador 1", labor: "Polinización", units: 35, verified: false, delegated: true, date: new Date().toLocaleDateString("es-EC") };
    setAdvances([item, ...advances]); notify("Avance delegado registrado");
  };
  ...
  return <div className="p-4 md:p-6 space-y-4 max-w-6xl mx-auto">
    <div><h1 className="text-xl md:text-2xl font-bold">Gestión agrícola</h1><p className="text-sm text-muted-foreground">Módulos añadidos para completar los requisitos Must del MVP.</p></div>
    <div className="flex gap-2 overflow-x-auto pb-1">{tabs.map(([id,label]) => <button key={id} onClick={()=>setTab(id)} className={`${btn} whitespace-nowrap ${tab===id?"bg-[#2D6A4F] text-white":"bg-card border border-border text-foreground"}`}>{label}</button>)}</div>
    {toast && <div className="bg-green-50 border border-green-200 text-green-700 rounded-xl px-4 py-3 text-sm font-semibold">✓ {toast}</div>}

    {tab==="variedades" && <div className={card}><div className="flex justify-between items-center mb-4"><div><h2 className="font-bold">Variedades y umbrales</h2><p className="text-xs text-muted-foreground">RF-10 · configura criterios por variedad.</p></div><button className={`${btn} bg-[#2D6A4F] text-white`} onClick={()=>setVarieties([...varieties,{id:Date.now(),nombre:"Nueva variedad",umbralMadurez:75,umbralPolinizacion:80}])}>+ Nueva variedad</button></div><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="text-left text-xs text-muted-foreground"><th className="p-2">Variedad</th><th className="p-2">Madurez %</th><th className="p-2">Polinización %</th><th className="p-2">Acción</th></tr></thead><tbody>{varieties.map((v:any)=><tr key={v.id} className="border-t"><td className="p-2 font-semibold">{v.nombre}</td><td className="p-2"><input type="number" value={v.umbralMadurez} onChange={e=>setVarieties(varieties.map((x:any)=>x.id===v.id?{...x,umbralMadurez:+e.target.value}:x))} className="w-20 border rounded-lg px-2 py-1"/></td><td className="p-2"><input type="number" value={v.umbralPolinizacion} onChange={e=>setVarieties(varieties.map((x:any)=>x.id===v.id?{...x,umbralPolinizacion:+e.target.value}:x))} className="w-20 border rounded-lg px-2 py-1"/></td><td className="p-2"><button className="text-red-600 font-bold" onClick={()=>setVarieties(varieties.filter((x:any)=>x.id!==v.id))}>Eliminar</button></td></tr>)}</tbody></table></div></div>}

    {tab==="nutricion" && <div className="grid md:grid-cols-2 gap-4"><div className={card}><h2 className="font-bold">Diagnóstico nutricional por imagen</h2><p className="text-xs text-muted-foreground mt-1">RF-08 · carga una foto y registra el diagnóstico.</p><input type="file" accept="image/*" className="mt-4 w-full border rounded-xl p-3"/><select className="mt-3 w-full border rounded-xl p-3"><option>Hoja</option><option>Planta completa</option><option>Suelo</option></select><button className={`${btn} bg-[#2D6A4F] text-white mt-3 w-full`} onClick={()=>notify("Diagnóstico nutricional: posible deficiencia de Magnesio · 87% confianza")}>Analizar imagen</button></div><div className={card}><h3 className="font-bold">Resultado del análisis</h3><div className="mt-4 p-4 bg-amber-50 rounded-xl"><p className="font-bold">Deficiencia de Magnesio</p><p className="text-sm">Confianza: 87%</p><p className="text-sm mt-2">Recomendación: aplicar sulfato de magnesio y repetir análisis en 21 días.</p></div><div className="mt-3 text-xs text-muted-foreground">Estado: simulado en el prototipo, pero flujo funcional de captura → análisis → recomendación.</div></div></div>}

    {tab==="produccion" && <div className="grid md:grid-cols-3 gap-4"><div className={`${card} md:col-span-2`}><h2 className="font-bold">Estimación de producción · RF-18</h2><div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4">{[["Lotes",14],["Racimos estimados",1240],["Toneladas",production],["Cumplimiento",Math.round(production/52*100)+"%"]].map(([a,b])=><div className="bg-background rounded-xl p-3" key={String(a)}><p className="text-xs text-muted-foreground">{a}</p><p className="text-xl font-bold mt-1">{b}</p></div>)}</div><label className="block text-xs font-bold mt-5">Ajustar estimación (T)</label><input type="range" min="20" max="100" value={production} onChange={e=>setProduction(+e.target.value)} className="w-full"/></div><div className={card}><h3 className="font-bold">Exportación</h3><p className="text-sm text-muted-foreground mt-1">Sin datos personales.</p><button className={`${btn} bg-[#2D6A4F] text-white mt-4 w-full`} onClick={()=>exportCsv([{lote:"Lote 1",toneladas:production,meta:52,cumplimiento:Math.round(production/52*100)+"%"}],"simpa_estimacion_produccion.csv")}>Exportar CSV</button></div></div>}

    {tab==="madurez" && <div className="grid md:grid-cols-2 gap-4"><div className={card}><h2 className="font-bold">Clasificación de madurez · RF-21</h2><div className="text-center py-5"><div className="text-6xl font-bold text-[#2D6A4F]">{maturity}%</div><p className="text-sm text-muted-foreground">Índice estimado de madurez</p></div><input type="range" min="0" max="100" value={maturity} onChange={e=>setMaturity(+e.target.value)} className="w-full"/><div className="mt-3 grid grid-cols-3 gap-2 text-center text-xs"><div className="bg-red-50 rounded-lg p-2">Verde<br/><b>18%</b></div><div className="bg-amber-50 rounded-lg p-2">Pintón<br/><b>36%</b></div><div className="bg-green-50 rounded-lg p-2">Maduro<br/><b>{maturity}%</b></div></div></div><div className={card}><h2 className="font-bold">RF-22 · Alerta de fruta verde</h2><p className="text-sm text-muted-foreground mt-1">Umbral preventivo configurable.</p><input type="range" min="0" max="100" value={greenFruit} onChange={e=>setGreenFruit(+e.target.value)} className="w-full mt-5"/><div className={`mt-4 p-4 rounded-xl ${greenFruit>20?"bg-red-50 text-red-700":"bg-green-50 text-green-700"}`}><b>{greenFruit}% de fruta verde</b><p className="text-sm mt-1">{greenFruit>20?"Alerta preventiva activa: revisar lote y programación de cosecha.":"Nivel dentro del umbral."}</p></div></div></div>}

    {tab==="planificacion" && <div className={card}><div className="flex justify-between items-center mb-4"><div><h2 className="font-bold">Planificación semanal con presupuesto · RF-26</h2><p className="text-xs text-muted-foreground">Incluye metas, presupuesto y estado.</p></div><button className={`${btn} bg-[#2D6A4F] text-white`} onClick={addPlan}>+ Crear semana</button></div><table className="w-full text-sm"><thead><tr className="text-left text-xs text-muted-foreground"><th className="p-2">Semana</th><th className="p-2">Lote</th><th className="p-2">Labor</th><th className="p-2">Meta</th><th className="p-2">Presupuesto</th><th className="p-2">Estado</th></tr></thead><tbody>{plans.length===0?<tr><td colSpan={6} className="p-6 text-center text-muted-foreground">No hay planes. Crea el primero.</td></tr>:plans.map((x:any)=><tr className="border-t" key={x.id}><td className="p-2">{x.semana}</td><td className="p-2">{x.lot}</td><td className="p-2">{x.labor}</td><td className="p-2">{x.meta}</td><td className="p-2">${x.presupuesto}</td><td className="p-2"><span className="px-2 py-1 rounded-full bg-green-100 text-green-700 text-xs font-bold">{x.estado}</span></td></tr>)}</tbody></table></div>}

    {tab==="avance" && <div className={card}><div className="flex justify-between items-center mb-4"><div><h2 className="font-bold">Registro delegado del avance · RF-35</h2><p className="text-xs text-muted-foreground">Permite que supervisor registre avance en nombre del operario.</p></div><button className={`${btn} bg-[#2D6A4F] text-white`} onClick={addAdvance}>+ Registrar delegado</button></div><div className="space-y-2">{advances.length===0?<p className="text-sm text-muted-foreground p-5 text-center">Aún no hay avances delegados.</p>:advances.map((x:any)=><div key={x.id} className="border rounded-xl p-3 flex flex-wrap gap-3 items-center"><b>{x.lot}</b><span>{x.worker}</span><span>{x.labor}</span><span>{x.units} unidades</span><span className="px-2 py-1 rounded-full bg-blue-100 text-blue-700 text-xs">Delegado</span><button className="ml-auto text-xs font-bold text-[#2D6A4F]" onClick={()=>setAdvances(advances.map((a:any)=>a.id===x.id?{...a,verified:!a.verified}:a))}>{x.verified?"✓ Verificado":"Marcar verificado"}</button></div>)}</div></div>}

    {tab==="tarifas" && <div className={card}><div className="flex justify-between items-center mb-4"><div><h2 className="font-bold">Catálogo de tarifas por labor · RF-36</h2><p className="text-xs text-muted-foreground">Tarifa por unidad para liquidación.</p></div><button className={`${btn} bg-[#2D6A4F] text-white`} onClick={()=>setTariffs([...tariffs,{id:Date.now(),labor:"Nueva labor",unidad:"unidad",tarifa:25}])}>+ Nueva tarifa</button></div><table className="w-full text-sm"><thead><tr className="text-left text-xs text-muted-foreground"><th className="p-2">Labor</th><th className="p-2">Unidad</th><th className="p-2">Tarifa</th><th className="p-2"></th></tr></thead><tbody>{tariffs.map((x:any)=><tr className="border-t" key={x.id}><td className="p-2 font-semibold">{x.labor}</td><td className="p-2">{x.unidad}</td><td className="p-2"><input type="number" value={x.tarifa} onChange={e=>setTariffs(tariffs.map((a:any)=>a.id===x.id?{...a,tarifa:+e.target.value}:a))} className="w-24 border rounded-lg px-2 py-1"/> USD</td><td className="p-2"><button className="text-red-600 font-bold" onClick={()=>setTariffs(tariffs.filter((a:any)=>a.id!==x.id))}>Eliminar</button></td></tr>)}</tbody></table></div>}

    {tab==="remuneracion" && <div className="grid md:grid-cols-3 gap-4"><div className={`${card} md:col-span-2`}><h2 className="font-bold">Cálculo de remuneración semanal · RF-37</h2><div className="mt-4 space-y-2">{tariffs.slice(0,3).map((t:any,i:number)=>{const units=[38,52,27][i]||20; return <div key={t.id} className="flex justify-between border-b py-3 text-sm"><span>{t.labor} · {units} {t.unidad}</span><b>${(units*t.tarifa).toFixed(2)}</b></div>})}</div><div className="mt-4 flex justify-between text-lg font-bold"><span>Total semanal</span><span>${tariffs.slice(0,3).reduce((s:number,t:any,i:number)=>s+([38,52,27][i]||20)*t.tarifa,0).toFixed(2)}</span></div></div><div className={card}><h3 className="font-bold">Aprobación de liquidación</h3><p className="text-sm text-muted-foreground mt-1">Revisión previa al cierre semanal.</p><button className={`${btn} mt-4 w-full ${approved?"bg-green-100 text-green-700":"bg-[#2D6A4F] text-white"}`} onClick={()=>{setApproved(!approved);notify(approved?"Liquidación reabierta":"Liquidación semanal aprobada")}}>{approved?"✓ Aprobada":"Aprobar liquidación"}</button><button className={`${btn} bg-muted mt-2 w-full`} onClick={()=>exportCsv([{semana:"Semana actual",total:tariffs.slice(0,3).reduce((s:number,t:any,i:number)=>s+([38,52,27][i]||20)*t.tarifa,0)}],"simpa_liquidacion_semanal.csv")}>Exportar liquidación</button></div></div>}
    {tab==="derechos" && <div className="grid md:grid-cols-3 gap-4"><div className={card}><h2 className="font-bold">RF-40 · Exportar mis datos</h2><p className="text-sm text-muted-foreground mt-1">La persona titular puede obtener sus datos sin intermediación administrativa.</p><button className={`${btn} bg-[#2D6A4F] text-white mt-4 w-full`} onClick={()=>exportCsv([{nombre:"Trabajador 1",cargo:"Operario de campo",lote:"Lote 1",telefono:"3001234567",avance:35}],"mis_datos_personales.csv")}>Descargar mis datos CSV</button><p className="text-[11px] text-muted-foreground mt-3">La solicitud queda registrada con marca temporal.</p></div><div className={card}><h2 className="font-bold">RF-41 · Rectificación con bitácora</h2><input defaultValue="Trabajador 1" className="w-full border rounded-xl p-2.5 mt-3"/><input placeholder="Nuevo valor" className="w-full border rounded-xl p-2.5 mt-2"/><input placeholder="Motivo de la corrección" className="w-full border rounded-xl p-2.5 mt-2"/><button className={`${btn} bg-[#2D6A4F] text-white mt-3 w-full`} onClick={()=>{const log={fecha:new Date().toLocaleString("es-EC"),autor:role,motivo:"Rectificación solicitada",valorAnterior:"Trabajador 1"};localStorage.setItem("simpa_bitacora_v1",JSON.stringify([log,...JSON.parse(localStorage.getItem("simpa_bitacora_v1")||"[]")]));notify("Rectificación guardada y valor anterior conservado en bitácora")}}>Guardar rectificación</button></div><div className={card}><h2 className="font-bold">RF-42 · Supresión y disociación</h2><p className="text-sm text-muted-foreground mt-1">Elimina identificativos y conserva el histórico productivo con ID disociado.</p><button className={`${btn} bg-red-600 text-white mt-4 w-full`} onClick={()=>{const ok=window.confirm("¿Confirmar supresión de datos identificativos? El histórico productivo se conservará disociado."); if(ok) notify("Datos identificativos suprimidos; histórico conservado con ID disociado")}}>Solicitar supresión</button><p className="text-[11px] text-muted-foreground mt-3">Acción irreversible en una implementación productiva; aquí se simula para demostración.</p></div></div>}


    <div className="text-xs text-muted-foreground bg-muted/50 rounded-xl p-3">Rol actual: <b>{role}</b>. En un despliegue real, autenticación, almacenamiento y modelos IA deben estar respaldados por backend; este prototipo conserva la persistencia local para demostración.</div>
  </div>;
}
